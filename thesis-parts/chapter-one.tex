...
Internet-of-Things (IoT), Smart City, reti di sensori o più in generale in sistemi
collettivi adattivi. Quando si parla di sistemi collettivi si deve tenere in
considerazione l'elevato numero di dispositivi diversi di cui essi sono
composti e l'eterogeneità che ne deriva in termini di piattaforme, tecnologie, paradigmi di
programmazione, protocolli di comunicazione, eccetera. Le specifiche di basso livello, come efficienza, organizzazione e coordinazione
di questi dispositivi in un sistema distribuito, influenzano pesantemente le
scelte di design di quest'ultimo. Ne può risultare un sistema rigido, costoso da
manutenere, estendere, ed eventualmente scalare in dimensione.
...
deployment. Possono essere elencate tre caratteristiche
...
\begin{figure}
  ...
  \caption{Lo spazio che ci circonda è denso di oggetti in grado di effettuare calcoli e capaci di comunicare. Alcuni di questi sono oggetti infrastrutturali, ma la maggior parte sono oggetti personali, in continuo movimento.}
...
\begin{figure}
  ...
  \caption{Operatori del livello di astrazione ``building block''. Questi
    quattro operatori sono la base fondante di tutte le API sviluppatore fornite
